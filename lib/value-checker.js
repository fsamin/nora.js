...
/**
  ...
  */
var checker = function doStepCheck(runningTestStep) {
  ...
  if (teststep.stepOptions.value == null || 
    teststep.stepOptions.asserts == null
    ) {
    runningTestStep.console.error("Error parsing " + teststep.stepID + " options.\n value, asserts are mandatory.\nPlease correct your json testcase before relaunch nora.js.");
    runningTestStep.console.dir(teststep);
    throw new Error("Malformated check test step");
  }

  var value = setXMLProperties(runningTestStep, teststep.stepOptions.value, teststep.stepOptions.namespaces, properties, debug);
  ...
  teststep.stepOptions.asserts.forEach(function(myAssert){
    ...
    switch(myAssert.type) {
      case "contains" :
        ...
        var tmpResult = (value.indexOf(setXMLProperties(runningTestStep, myAssert.value, myAssert.namespaces, properties, debug)) > -1);
