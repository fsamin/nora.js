  ...
  teststep.stepOptions.asserts.forEach(function(myAssert){
    ...
    switch(myAssert.type) {
      case "contains" :
        ...
        if (!tmpResult) {
          //Test regex if not found
          var pattern = new RegExp(setXMLProperties(runningTestStep, myAssert.value, myAssert.namespaces, properties, debug));
          var arrMatches = xmlFile.match(pattern);
          tmpResult = (arrMatches != null);
        }
